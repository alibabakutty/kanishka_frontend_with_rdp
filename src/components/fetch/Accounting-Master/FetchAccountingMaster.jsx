...
const FetchAccountingMaster = () => {
  ...
  const filteredCustomers = customers.filter((customer) => {
    ...
    return (
      customer.sundryCreditorName?.toLowerCase().includes(search) ||
      customer.parentName?.toLowerCase().includes(search) ||
      customer.grandParentName?.toLowerCase().includes(search)
    );
  });
  ...
  const handleKeyDown = useCallback((e) => {
    ...
    switch (e.key) {
      ...
      case 'Enter': {
        ...
        if (selectedCustomer) {
          navigate(`/update_customer/${selectedCustomer.id}`);
        }
        break;
      }
      default:
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        ...
        const response = await fetch(`${API_URL}/api/v1/accounting-masters`, {
          method: 'GET',
          headers: {
        ...
        <table className="w-full border-collapse border border-gray-300">
          <thead className="bg-[#004d26] text-white text-left [&_th]:px-[0.5] [&_th]:py-1 [&_th]:border [&_th]:border-gray-500">
            <tr className="bg-[#004d26] text-white text-left  ">
              ...
              <th className="border border-gray-400 text-center">Parent Name</th>
              <th className="border border-gray-400 text-center">Grand Parent Name</th>
            </tr>
          </thead>
          ...
          <tbody className="[&_th]:px-1 [&_th]:py-1 [&_th]:border [&_th]:border-gray-500" >
            {filteredCustomers.length > 0 ? (
              filteredCustomers.map((customer, rowIndex) => {
                const rowData = [
                  rowIndex + 1,
                  customer.sundryCreditorName,
                  customer.parentName,
                  customer.grandParentName
                ];
                ...
                return (
                  <tr
                    ...
                    key={customer.id}
                    onClick={() => navigate(`/update_customer/${customer.id}`)}
...
export default FetchAccountingMaster;
